...
/// Exports the stable fields discovered in Apple's ZCALLRECORD Core Data table.
/// Dates are converted from Apple's 2001 epoch to the Mac's local timezone.
pub fn export_calls_csv(database_path: &Path, output_path: &Path) -> Result<u64> {
    ...
    let mut statement = connection.prepare(
        "SELECT ZDATE, ZDURATION, ZORIGINATED, ZANSWERED, ZADDRESS, ZNAME, \
                ZCALLTYPE, ZISO_COUNTRY_CODE, ZSERVICE_PROVIDER, ZUNIQUE_ID \
         FROM ZCALLRECORD \
         ORDER BY ZDATE ASC",
    )?;
    ...
    let mut count = 0_u64;
    for row in rows {
        let (date, duration, originated, answered, address, name, call_type, country, provider, unique_id) = row?;
        let date_text = date.and_then(format_apple_timestamp).unwrap_or_default();
        let direction = match originated {
            Some(1) => "ausgehend",
            Some(_) => "eingehend",
            None => "unbekannt",
        };
        let answered_text = match answered {
            Some(1) => "ja",
            Some(_) => "nein",
            None => "unbekannt",
        };
...
fn format_apple_timestamp(value: f64) -> Option<String> {
    ...
    let nanos = ((unix_seconds - seconds as f64) * 1_000_000_000.0)
        .round()
